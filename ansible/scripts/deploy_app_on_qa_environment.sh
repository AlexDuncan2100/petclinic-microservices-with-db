...
AWS_REGION=$AWS_REGION helm repo add stable-petclinica s3://adu-helm/stable/myapp/ || echo "repository name already exists"
AWS_REGION=$AWS_REGION helm repo update
helm package k8s/petclinic_chart
AWS_REGION=$AWS_REGION helm s3 push petclinic_chart-${BUILD_NUMBER}.tgz stable-petclinica
envsubst < ansible/playbooks/qa-petclinic-deploy-template >ansible/playbooks/qa-petclinic-deploy.yaml
ansible-playbook -i ./ansible/inventory/qa_stack_dynamic_inventory_awa